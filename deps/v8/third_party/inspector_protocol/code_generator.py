...
class Protocol(object):
  ...
  def check_if_dependency_declared(self, domain, refs):
    ...
    for ref in refs:
      ...
      if type_definition.get('is_primitive', False):
        continue
      if domain_match := re.match(r'^(.*)[.]', ref):
        referenced_domain_name = domain_match[1]
        if (referenced_domain_name != domain['domain']
            and referenced_domain_name not in dependencies):
          sys.stderr.write((
  ...
  def generate_command(self, domain, command):
    return (self.check_options(
        ...
        "exclude",
        True,
    ) if self.config.protocol.options else domain in self.generate_domains)
  ...
  def is_async_command(self, domain, command):
    return (self.check_options(self.config.protocol.options, domain, command,
                               "async_", None, False)
            if self.config.protocol.options else False)
  ...
  def is_imported(self, domain, name):
    ...
    return (self.check_options(self.config.imported.options, domain, name,
                               "imported", None, False)
            if self.config.imported.options else domain in self.imported_domains)
